...
class GurobiTspSolver:
    ...
    def _enforce_vertex_constraints(self):
        #add constraints that every node is entered and left exactly once
    ...
    def solve(self, time_limit: float, opt_tol: float = 0.001) -> None:
        ...
        def callback(model, where):
            if where == gp.GRB.Callback.MIPSOL:
                ...
                if len(comps) == 1:
                    return # tour is already whole graph
                ...
                for comp in comps:
                    ...
                    if len(comp) < 2:
                        continue
                    model.cbLazy(sum(x for _, x in self._edge_vars.outgoing_edges(comp)) >= 2)
